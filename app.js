...
app.post('/recommendations', (req, res) => {
    const favMovies = req.body.favMovies;

    // Execute the Python script as a child process
    exec(`python python_script/movie_recommendation.py "${favMovies.join(',')}"`, (error, stdout, stderr) => {
        if (error) {
            console.error('Error executing Python script:', error);
            res.status(500).json({ error: 'Internal Server Error' });
        } else {
            // Parse the movie recommendations from the Python script's output
